...
def _build_choices(correct, all_names, max_choices=4):
    """
    Build answer choices: correct + up to (max_choices-1) random others.
    Returns shuffled list.
    """
    other = [n for n in all_names if n != correct]
    random.shuffle(other)
    choices = [correct] + other[:max_choices - 1]
    random.shuffle(choices)
    return choices

def index(request):
    """Home page with start quiz forms and random fact."""
...
def result_view(request):
    """Universal result page for text quiz."""
    ...
    # Clean any leftover quiz data
    for key in list(request.session.keys()):
        if key.startswith('text_') or key == 'player_name':
...
def question_view(request):
    """Display text question and process answer."""
    ...
    choices = _build_choices(correct, all_species)
...
def photo_question_view(request):
    """Display photo question and process answer."""
    ...
    choices = _build_choices(correct, all_species_names)
...
def photo_result_view(request):
    """Display final result for photo quiz."""
